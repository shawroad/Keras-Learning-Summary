...
"""
# 参考自: https://kexue.fm/archives/5861
...
def data_generator():
    ...
    while True:
        for t, c in zip(titles, texts):
            X.append(str2id(c))
            Y.append(str2id(t, start_end=True))   # 只需给标题加开始和结尾
            if len(X) == batch_size:
...
def to_one_hot(x):
    ...
    x = K.one_hot(x, len(chars) + 4)   # 转one_hot
